...
begin
  PersistentClass := ServerMethodsUnit1.TTabelaVenda;
end;
...
{$REGION 'Buscando a sessão no REDIS, aumentando o TTL'}
        ...
        Result   := Self.Redis.EXISTS(sKeyName);
        if Result then
        begin
          aDataSession := Self.Redis.HMGET(sKeyName, aKeys);
          Self.Redis.EXPIRE(sKeyName, 3600);
